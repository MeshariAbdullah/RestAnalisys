...
import { detectLateReturns } from "./services/lateReturnService.js";
...
app.listen(PORT, () => {
  ...
  // Run late return detection every hour
  detectLateReturns().then((count) => {
    if (count > 0) console.log(`[startup] ${count} late rental(s) detected`);
  });
  setInterval(() => {
    detectLateReturns().catch((err) => console.error("[late-returns] check failed:", err));
  }, 60 * 60 * 1000);
});
