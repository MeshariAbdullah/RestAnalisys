...
import { notifyRentalCreated, notifyRentalStatusChange } from "../services/notificationService.js";
...
async function buildRiskFeatures(userId: number, assetValueHalalas: number): Promise<RiskFeatures> {
  ...
  const stats = await db
    .select({
      ...
      lateReturns: sql<number>`count(*) filter (where status in ('closed','closed_with_penalty') and returned_at is not null and returned_at::date > end_date::date)`,
    })
    .from(rentals)
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    notifyRentalCreated(renter!.email, rental.reference).catch(() => {});
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    const [renterUser] = await db.select({ email: users.email }).from(users).where(eq(users.id, rental.renterId)).limit(1);
    if (renterUser) notifyRentalStatusChange(renterUser.email, rental.reference, "active").catch(() => {});
