  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    if (result.status === "captured") {
      await notify({
        ...
        message: `Payment captured for rental ${rental.reference}. Your asset will be shipped to the renter.`,
        entityType: "rental",
        entityId: rental.id,
      });
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    await notify({
      ...
      message: `A payout of ${(payoutCalc.netHalalas / 100).toFixed(2)} SAR has been released for rental ${rental.reference}.`,
      entityType: "rental",
      entityId: rental.id,
    });
