...
means denied. Permission keys use **dot notation** (e.g. `asset.submit`).
...
Routes that enter into legal contracts or create a rental also use
`requireNafath`:

- `POST /api/rentals` (rental creation)
- `POST /api/legal/sign` (legal commitment signing)
