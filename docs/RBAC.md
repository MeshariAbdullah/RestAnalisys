...
| `super_admin` | Admin + ability to create staff users and impersonate       |

## Permission matrix

This is the source of truth. A check means that role has the permission.
Permission identifiers use **dot notation** (e.g. `asset.submit`) — this
matches the code in `rbac.ts`.
...
Unverified users hitting these endpoints receive a 403 with a hint to
complete Nafath before retrying.
