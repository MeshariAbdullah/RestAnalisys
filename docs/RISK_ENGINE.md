...
| Disputed rentals (per dispute)   | -10     |
| Late returns (per late return)   | -5      |
| ≥ 3 cancelled rentals            | -8      |
| Phone verified                   | +2      |
| Email verified                   | +2      |
| Non-KSA identity                 | -10     |
...
- `requiresReview` → same, but rental status is `pending_risk_review`
...
- The feature snapshot columns (account age, completed rentals, etc.)
- The computed base/final score and risk category
- The list of modifiers as JSON
