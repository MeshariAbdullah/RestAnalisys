...
and is unit-testable without a database (tests in `riskEngine.test.ts`).
...
interface RiskDecision {
  ...
  riskCategory: RiskCategory;        // "low" | "medium" | "high" | "ultra_high"
...
- `approved && requiresReview` -> same, but rental status is `pending_risk_review`
