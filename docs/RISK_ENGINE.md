...
| Disputed rentals (each)          | -10     |
| Late returns (each)              | -5      |
| >= 3 cancellations               | -8      |
| Phone verified                   | +2      |
| Email verified                   | +2      |
| Non-Saudi identity               | -10     |
...
**flagged for manual review** — the rental is approved but
`requiresReview` is set to true.
...
- `approved && !requiresReview` -> create rental + legal commitment
- `approved && requiresReview` -> same, flagged for review
- `!approved` -> throw `RiskRejectionError` -> HTTP 422 with reason
...
- The computed score and modifiers
- The risk category and approval status
