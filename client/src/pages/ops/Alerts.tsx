...
export default function AlertsPage() {
  ...
  async function resolve(id: number) {
    try {
      ...
      toast({ title: "Alert resolved", description: "The alert has been marked as resolved.", variant: "success" });
    } catch (err) {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <p className="text-neutral-500 mb-8">Risk events detected by the platform that need human attention.</p>
              ...
              <CardContent className="p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge className={`border-0 ${severityColor(a.severity)}`}>{a.severity}</Badge>
                    <Badge variant="outline">{a.status}</Badge>
                  </div>
                  <p className="text-sm text-neutral-600 mt-1">{a.message}</p>
                  <p className="text-xs text-neutral-400 mt-2">{new Date(a.createdAt).toLocaleString()}</p>
                </div>
                {a.status === "open" && (
