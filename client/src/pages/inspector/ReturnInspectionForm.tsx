...
import { assetsApi, rentalsApi, inspectionsApi } from "@/lib/api";
...
export default function ReturnInspectionForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!rental) {
      setError("No rental under inspection found for this asset");
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-1">
        {asset
          ? `Return inspection — ${asset.brand} ${asset.title}`
          : "Return inspection"}
      </h1>
      <p className="text-neutral-500 mb-2">
        Inspect the returned asset and assess its condition after the rental
        period.
      </p>
      {rental && (
        <p className="text-sm text-neutral-500 mb-8">
          Rental #{rental.reference} · {rental.startDate} — {rental.endDate}
        </p>
      )}
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <section>
              ...
              <Textarea
                ...
                placeholder="Describe any new scratches, scuffs, missing parts, damage..."
                rows={4}
                className="mt-3"
              />
            </section>
