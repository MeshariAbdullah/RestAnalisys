...
export default function ReturnInspectionForm({ assetId }: { assetId: number }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!rental) {
      setError("No rental found under inspection for this asset");
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <p className="text-neutral-500 mb-2">
        Inspect the returned asset and assess any damage.
      </p>
      {rental && (
        <p className="text-sm text-neutral-500 mb-8">
          Rental <span className="font-mono">{rental.reference}</span> · {rental.durationDays} days
        </p>
      )}

      {!rental && !submitting && allRentals && (
        <div className="mb-6 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded p-3">
          No rental with status "under_inspection" found for this asset.
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardContent className="p-6 space-y-6">
            <section>
              <h2 className="font-semibold mb-3">Condition after return</h2>
              ...
              <Textarea
                ...
                placeholder="Describe any new scratches, scuffs, stains, missing components…"
                rows={4}
                className="mt-3"
              />
            </section>
            ...
            <section>
              <h2 className="font-semibold mb-3">Post-return valuation</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
