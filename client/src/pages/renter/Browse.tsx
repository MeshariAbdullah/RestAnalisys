...
export default function Browse() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => assetsApi.listings(queryParams),
  });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-4 p-4 bg-neutral-50 rounded-lg border">
            <div className="flex items-center gap-2">
              <label className="text-sm font-medium text-neutral-600">Daily Price (SAR):</label>
              <Input
                type="number"
                placeholder="Min"
                value={minPrice}
      ...
      {isLoading ? (
        ...
        <>
          <p className="text-sm text-neutral-500 mb-4">{items.length} item{items.length !== 1 ? "s" : ""} found</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((asset) => (
