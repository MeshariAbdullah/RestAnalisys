...
const SORTS = [
  ...
  { id: "price_asc" as const, label: "Price: Low → High" },
  { id: "price_desc" as const, label: "Price: High → Low" },
];
...
export default function Browse() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => assetsApi.listings({ category, search: search || undefined, sort }),
  });
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          {SORTS.map((s) => (
            <Button
              ...
              className={sort === s.id ? "bg-neutral-800 text-white h-7 text-xs" : "h-7 text-xs"}
            >
              {s.label}
            </Button>
          ))}
          {items.length > 0 && (
            <span className="ml-auto text-sm text-neutral-500">{items.length} items</span>
          )}
        </div>
