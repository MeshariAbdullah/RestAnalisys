...
const STATUS_META: Record<string, { color: string; icon: typeof Clock; label: string }> = {
  ...
  enforcement: { color: "bg-red-200 text-red-800", icon: AlertCircle, label: "Enforcement" },
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {data.map((r: Rental) => (
            <Link key={r.id} href={`/rentals/${r.id}`} className="block">
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between gap-6 flex-wrap">
                    ...
                    <div className="text-right flex items-start gap-3">
                      ...
                      <ArrowRight className="w-5 h-5 text-neutral-400 mt-2" />
                    </div>
                  </div>
                </CardContent>
              </Card>
